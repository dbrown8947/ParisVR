...
public class LoadRoads : MonoBehaviour
{
	...
	/*void OnDrawGizmos()
	{
		...
		int count = 0;
		foreach (GameObject road in listOfTmp)
		{
			count++;
			foreach (Transform point in road.transform)
			{
				t.Add(point);
			}
			...
			controlPointsList = t.ToArray();

			for (int i = 1; i < controlPointsList.Length; i++)
			{
				//var current = t[i - 1].transform.position;
				//var next = t[i].transform.position;

				//Debug.DrawLine(current, next, Color.red);
